...
function sendEmail(){
	if($("#memNm1").val().length!=0 && $("#memId1").val().length!=0){
		alert("입력하신 이메일로 인증번호가 발송되었습니다.");
	}
}
function sendPhone(){
	if($("#memNm2").val().length!=0 && $("#memId2").val().length!=0){
...
<body>
	...
	<div class="accordionMenu">
		...
		<div id="css3" class="menuSection">
			...
			<div id="move2">
				<form name="findPwdByEmailfrm" id="findPwdByEmail"
					action="findPwdByEmail.do" method="post">
					<div class="inputEnter" style="display: block;">
						<div class="inputBox">
							<div class="styleInput">
								<input name="memNm" id="memNm1" type="text" class="iInput"
									required placeholder="이름">
							</div>
							<div class="styleInput">
								<input name="memId" id="memId1" type="text" class="iInput"
									required placeholder="아이디">
							</div>
							<div class="emailInput">
								<div class="styleInput">
									<input name="memEmail" id="memEmail" type="email"
										class="iInput" required placeholder="이메일">
								</div>
							</div>
						</div>
						<div class="btnArea">
							<button type="submit" class="btn8f" onclick="sendEmail();">인증번호 전송</button>
						</div>
					</div>
				</form>
			</div>
		...
		<div id="api" class="menuSection">
			...
			<div id="move2">
				<form name="findPwdByPhonefrm" id="findPwdByPhone"
					action="findPwdByPhone.do" method="post">
					<div class="inputEnter" style="display: block;">
						<div class="inputBox">
							<div class="styleInput">
								<input id="memNm2" name="memNm" type="text" class="iInput"
									required placeholder="이름">
							</div>
							<div class="styleInput">
								<input name="memId" id="memId2" type="text" class="iInput"
									required placeholder="아이디">
							</div>
							<div class="styleInput">
								<input id="inputHp" name="inputHp" type="text" class="iInput"
									required placeholder="휴대폰번호 (-없이 입력)">
							</div>
						</div>
						<div class="btnArea">
							<button type="submit" class="btn8f" onclick="sendPhone();">인증번호 전송</button>
						</div>
					</div>
				</form>
			</div>
